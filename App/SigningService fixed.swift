...
final class SigningService: ObservableObject {
    ...
    /// Blocking sign call wrapper that runs the C++ signing on a background GCD queue and
    ...
    nonisolated static func signWithTimeout(
        ipa: URL,
        p12: URL,
        password: String,
        profile: URL,
        ...
        // Local result storage (value types only)
        var ok = false
        var message = ""
        var signedId = ""
        var signedVersion = ""

        // Semaphore to wait for background completion or timeout
        let sem = DispatchSemaphore(value: 0)

        // Run the blocking C++ signing call on a background queue.
        // Pass only the value parameters; avoid capturing non-Sendable state.
        DispatchQueue.global(qos: .userInitiated).async {
            // Prepare C buffers (same sizes as sign())
            var msgBuf = [CChar](repeating: 0, count: 1024)
            var bidBuf = [CChar](repeating: 0, count: 512)
            var verBuf = [CChar](repeating: 0, count: 128)

            // Call into the native signing bridge (blocking)
            ...
            // Decode C strings safely
            let decodedMessage = String(decoding: msgBuf.prefix(while: { $0 != 0 }).map { UInt8(bitPattern: $0) }, as: UTF8.self)
            let decodedId = String(decoding: bidBuf.prefix(while: { $0 != 0 }).map { UInt8(bitPattern: $0) }, as: UTF8.self)
            let decodedVer = String(decoding: verBuf.prefix(while: { $0 != 0 }).map { UInt8(bitPattern: $0) }, as: UTF8.self)

            ok = (status == 0)
            message = decodedMessage.isEmpty ? (ok ? "Signed." : "Failed.") : decodedMessage
            signedId = decodedId
            signedVersion = decodedVer.isEmpty ? "1.0" : decodedVer

            // Signal completion
            sem.signal()
        }

        // Wait up to timeout for the background work to finish
        let waitResult = sem.wait(timeout: .now() + timeout)
        if waitResult == .timedOut {
            // Timed out: return failure message. Note: the background worker may still be running.
            return (false, "Signing timed out after \(Int(timeout)) seconds.", "", "")
        }

        return (ok, message, signedId, signedVersion)
    }
